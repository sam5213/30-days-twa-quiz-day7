...
bot.start((ctx) => {
    ctx.reply('Добро пожаловать в квиз! Нажмите на кнопку ниже, чтобы начать.', Markup.keyboard([
        [Markup.button.webApp('Начать квиз', 'https://sam5213.github.io/30-days-twa-quiz-day7')]
    ]));
...
bot.on('web_app_data', (ctx) => {
    ...
    try {
        ...
        const message = `Пользователь ${currentUser ? ctx.from.username : currentUser} прошел квиз. В качестве ответов выбирал: ${parsedData.results}`; // Формируем сообщение
        console.log(message);
